...
$forum_q=$topic_id?'':sanitize_text_field(wp_unslash($_GET['forum_q']??''));$forum_by=max(1,min(4,(int)($_GET['forum_by']??1)));$topic_q=$topic_id?sanitize_text_field(wp_unslash($_GET['topic_q']??'')):'';
...
elseif($section){
    $args['meta_query']=[['key'=>'_thai_forum_section','value'=>TOP_Forum_Services::section_ids($section)??[$section],'compare'=>'IN','type'=>'NUMERIC']];
    if($forum_q!==''){
...
$topic_suffix=($topic_id&&$topic_q!=='')?'?'.http_build_query(['topic_q'=>$topic_q]):'';
...
<?php TOP_Forum_Services::navigation(); ?><br>
...
<table class="ThrTopButtonsTbl" border="0" width="100%" cellspacing="0" cellpadding="0"><tr class="ThrTopButtonsRow1"><td width="50%" class="ThrTopButtonsCl11"><?php $forum_switches($forum_total_pages,$current_page,$forum_page_url,$topic_suffix); ?></td><td align="right" class="frmBtns ThrTopButtonsCl12"><div><a href="#forum-form"><img alt="" style="margin:0;padding:0;border:0" title="Ответить" src="/img/forum_buttons/t_reply.png"></a> <a href="<?php echo esc_url('/forum/'.$section.'#forum-form'); ?>"><img alt="" style="margin:0;padding:0;border:0" title="Новая тема" src="/img/forum_buttons/t_new.png"></a></div></td></tr></table>
<table class="ThrForumBarTbl" border="0" width="100%" cellspacing="0" cellpadding="0"><tr class="ThrForumBarRow1"><td class="forumNamesBar ThrForumBarCl11 breadcrumbs" style="padding-top:3px;padding-bottom:5px"><a class="forumBar breadcrumb-item" href="/forum/">Форум</a><?php foreach($crumbs as $crumb)echo ' <span class="breadcrumb-sep">&raquo;</span> <a class="forumBar breadcrumb-item" href="/forum/'.(int)$crumb['id'].'">'.esc_html($crumb['name']).'</a>'; if($topic_post){$desc=get_post_meta($topic_post->ID,'_thai_forum_description',true);echo ' <span class="breadcrumb-sep">&raquo;</span> <a class="forumBarA" href="'.esc_url(str_replace('{page}',(string)$current_page,$forum_page_url).$topic_suffix).'">'.esc_html($topic_post->post_title).'</a>';if($desc)echo ' <span class="thDescr">('.esc_html($desc).')</span>';} ?></td><td class="ThrForumBarCl12"><form id="thread_search_form" action="<?php echo esc_url(str_replace('{page}','1',$forum_page_url)); ?>" method="get"><input class="fastNav" type="submit" value="Ok" id="thread_search_button"><input type="text" name="topic_q" value="<?php echo esc_attr($topic_q); ?>" placeholder="Поиск по теме" id="thread_search_field"></form></td></tr></table>
<?php elseif($section): ?>
...
<div class="gDivLeft"><div class="gDivRight"><table class="gTable threadpage-posts-table" width="100%" cellspacing="1" cellpadding="0"><tr><td class="gTableTop"><span class="forum-title"><?php the_title(); ?></span></td></tr><tr><td class="postSeparator"></td></tr>
<?php if(get_the_content()&&$current_page===1)echo '<tr><td class="thai-forum-post-wrap"><div class="ucoz-forum-post">'.wp_kses_post(wpautop(get_the_content())).'</div></td></tr>'; $comment_args=['post_id'=>$post_id,'status'=>'approve','orderby'=>'comment_date','order'=>'ASC','number'=>20,'offset'=>($current_page-1)*20];if($topic_q!==''){$comment_args['search']=$topic_q;$comment_args['search_columns']=['comment_content'];}$comments=get_comments($comment_args);$number=($current_page-1)*20;
foreach($comments as $c){$number++;$legacy=(int)get_comment_meta($c->comment_ID,'_ucoz_forum_post',true);$anchor=$legacy?'post'.$legacy:'comment'.$c->comment_ID;$avatar=get_comment_meta($c->comment_ID,'_thai_forum_avatar',true);$rank=get_comment_meta($c->comment_ID,'_thai_forum_rank',true);$group=get_comment_meta($c->comment_ID,'_thai_forum_group',true);$posts_legacy=get_comment_meta($c->comment_ID,'_thai_forum_posts',true);$reputation=get_comment_meta($c->comment_ID,'_thai_forum_reputation',true);$group_icon=get_comment_meta($c->comment_ID,'_thai_forum_group_icon',true);$attachments=get_comment_meta($c->comment_ID,'_thai_forum_attachments',true); ?>
<tr id="<?php echo esc_attr($anchor); ?>"><td class="thai-forum-post-wrap"><table class="postTable" width="100%" cellspacing="1" cellpadding="2"><tr><td class="postTdTop" width="23%" align="center"><span class="postUser"><?php echo esc_html($c->comment_author); ?></span></td><td class="postTdTop">Дата: <?php echo esc_html(get_comment_date('d.m.Y H:i',$c)); ?> | Сообщение # <a href="#<?php echo esc_attr($anchor); ?>"><?php echo $number; ?></a></td></tr><tr><td class="postTdInfo" valign="top"><?php if($avatar)echo '<img class="userAvatar" src="'.esc_url($avatar).'" alt="'.esc_attr($c->comment_author).'">';if($rank)echo '<div class="postRankName">'.esc_html($rank).'</div>';echo '<div class="postRankIco">';if($group_icon)echo '<img alt="" src="'.esc_url($group_icon).'">';echo '</div>';if($group)echo '<div class="postUserGroup">'.esc_html($group).'</div>';if($posts_legacy)echo '<div class="numposts">'.esc_html($posts_legacy).'</div>';if($reputation)echo '<div class="reputation">'.esc_html($reputation).'</div>';echo '<div class="statusBlock" aria-hidden="true">&nbsp;</div>'; ?></td><td class="posttdMessage" valign="top"><span class="ucoz-forum-post"><?php echo wp_kses_post($c->comment_content); ?></span><?php echo wp_kses_post($attachments); ?></td></tr><tr><td class="postBottom" align="center">&nbsp;</td><td class="postBottom"><table width="100%" cellspacing="0" cellpadding="0"><tr><td><a href="#forum-form">Ответить</a></td><td style="text-align:end;padding-inline-end:15px"></td><td width="2%" nowrap style="text-align:end"><a class="goOnTop" href="#land-full"><img alt="" style="margin:0;padding:0;border:0" src="/img/forum_buttons/p_up.png" title="Вверх"></a></td></tr></table></td></tr></table></td></tr><?php if($number===(($current_page-1)*20+1))echo '<tr class="thai-forum-legacy-ad-row"><td></td></tr>'; ?>
<?php } ?></table></div></div>
<?php TOP_Community::pagination($topic_comment_total,20,$current_page,'/forum/'.$section.'-'.$topic_id.'-{page}'.$topic_suffix);}wp_reset_postdata();endif; ?>
