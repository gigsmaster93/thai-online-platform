...
$section=(int)get_query_var('top_section');$id=(int)get_query_var('top_id');$page=TOP_Community::page();$sections=get_option('thai_forum_sections',[]);
$args=['post_type'=>'thai_forum_topic','post_status'=>'publish','posts_per_page'=>30,'paged'=>$page,'orderby'=>'modified','order'=>'DESC'];
if($id){$args['meta_key']='_ucoz_forum_id';$args['meta_value']=$id;$args['posts_per_page']=1;}
elseif($section){$args['meta_key']='_thai_forum_section';$args['meta_value']=$section;}
$q=($id||$section)?new WP_Query($args):null;if($id&&!$q->have_posts())status_header(404);
get_header(); ?>
...
<h1><?php the_title(); ?></h1>
<?php $post_id=get_the_ID();$comments=get_comments(['post_id'=>$post_id,'status'=>'approve','orderby'=>'comment_date','order'=>'ASC','number'=>20,'offset'=>($page-1)*20]);
...
<?php }TOP_Community::pagination(get_comments_number($post_id),20,$page,'/forum/'.$section.'-'.$id.'-{page}');}wp_reset_postdata();endif; ?>
</div>
<?php get_footer(); ?>