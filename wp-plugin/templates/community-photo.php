...
$id=(int)get_query_var('top_id');$album=(int)get_query_var('top_section');$page=TOP_Community::page();
...
if($id){$args['meta_key']='_ucoz_photo_id';$args['meta_value']=$id;$args['posts_per_page']=1;}
elseif($term)$args['tax_query']=[['taxonomy'=>'thai_photo_album','terms'=>$term->term_id,'include_children'=>true]];
$q=new WP_Query($args);if($id&&!$q->have_posts())status_header(404);
get_header(); ?>
...
if($id){echo '<h1>'.esc_html(get_the_title()).'</h1><a href="'.esc_url($full).'" target="_blank" rel="noopener"><img class="thai-photo-full" src="'.esc_url($full).'" alt="'.esc_attr(get_the_title()).'"></a><div>'.wp_kses_post(wpautop(get_the_content())).'</div>';}
else { ?>
...
<?php }}wp_reset_postdata();if(!$id){echo '</ul></div>';TOP_Community::pagination($q->found_posts,50,$page,($term?get_term_meta($term->term_id,'_thai_legacy_url',true):'/photo/').'?page{page}');} ?>
</div></div></div>
<?php get_footer(); ?>