...
foreach($rows as $r){if(in_array((int)$r['id'],[510,511],true))continue;$ids=get_posts(['post_type'=>'thai_excursion','post_status'=>'publish','numberposts'=>1,'fields'=>'ids','meta_key'=>'_ucoz_shop_id','meta_value'=>$r['id']]);if(!$ids)WP_CLI::error('Published product missing: '.$r['id']);$id=$ids[0];$backup[$id]=['_thai_transport_variants'=>get_post_meta($id,'_thai_transport_variants',true),'_thai_order_type'=>get_post_meta($id,'_thai_order_type',true),'_thai_person_labels'=>get_post_meta($id,'_thai_person_labels',true)];$updates[$id]=$r;}
...
foreach($updates as $id=>$r){update_post_meta($id,'_thai_order_type',$r['type']);update_post_meta($id,'_thai_transport_variants',$r['transport']??'');$labels=array_values(array_filter(array_map('trim',$r['labels']),'strlen'));update_post_meta($id,'_thai_person_labels',$labels);}
